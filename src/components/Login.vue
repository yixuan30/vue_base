<template>
  <div id="login">
    <h1 style="position: absolute;color: #fff;left: 50%;transform: translateX(-50%); top: -110px;">XXX-后台管理系统</h1>
    <el-form
      :model="userLoginForm"
      :rules="loginRules"
      status-icon
  ...
  methods: {
    ...
    async success () {
      ...
      const { data: res } = await this.$http.post(
        'user/login',
        {username:this.userLoginForm.username,password:this.userLoginForm.password}
      )
