  ...
  <div id="users">
    ...
    <el-card class="box-card">
      ...
      <el-table v-loading="loading" size="mini" :data="userList" border style="width: 100%;" height="420">
        ...
        <el-table-column label="#" prop="userId" width="180"></el-table-column>
        ...
        <el-table-column label="操作" >
          <template slot-scope="scope">
            <el-button   v-hasPermission="'user:edit'" size="mini" type="primary" icon="el-icon-edit-outline" @click="edit(scope.row.userId)"></el-button>
            <el-button v-hasPermission="'user:delete'" type="danger" size="mini" icon="el-icon-delete" @click="del(scope.row.userId)"></el-button>
            <el-tooltip
              class="item"
              effect="dark"
              content="分配角色"
...
export default {
  ...
  methods: {
    ...
    async getUserList () {
      ...
      this.userList = res.data.userList.rows
    },
    /**
     * 删除用户
     */
     ...
     */
    async edit (userId) {
      const { data: res } = await this.$http.get('user/edit/' + userId)
      if (res.code === 200) {
    ...
    async changUserStatus (row) {
      const { data: res } = await this.$http.put(
        'user/updateStatus/' + row.userId + '/' + row.status
      )
      if (res.code !== 200) {
